...
管线加入 G2 机器人支持（robot profile、snapshot 契约校验、头部度量深度、自遮挡掩膜、
诊断脚本），并在 13 个真实 snapshot 上做了四组实验。

完整记录见 [`G2_FINDINGS_20260722.md`](G2_FINDINGS_20260722.md)。要点：

- G2 直接提供 `base_T_camera`（OpenCV RDF、米、base_link），FK 与 SDK TF 差 0.0000 mm，
  无需再推导腕相机外参。
- 头部深度可用且干净（平面度 3.2–4.8 mm RMS），成为尺度参考。
- **重建误差不是尺度问题**：仿射拟合 `真实 = a·模型 + b` 的 `b` 达 300–614 mm 且逐视图不同，
  现行三条 baseline 的尺度拟合对此完全无感。全局仿射也拟合不上（内点率 32.6%）。
- **唯一有效的干预是 `--depth-input`**（喂度量深度）；遮挡机器人再推理、强制标定位姿都更差。
- **hand_right 的横向误差是位姿类误差**，喂深度修不了，且随 capture 变化（非固定外参错）。
  最大的结构性不对称是**与 head 的光轴夹角 39–42°，而 hand_left 只有 26–28°**。
- 参考系自身有 1.1–1.3° 的系统倾斜，构成所有测量的精度上限。

下一步优先级最高的实验：**交换 `VIEW_NAMES` 中两路手部相机的顺序**，用于区分
"视角过斜（几何）"与"视图序号（MapAnything 算法）"两个候选根因。
