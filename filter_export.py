...
def fallback_colors(capture, name, target_hw):
...
def process_capture(
    capture, max_radius=None, bbox=None, min_conf=None, show_cameras=False
):
    ...
    for name in VIEW_NAMES:
        ...
        if f"{name}_img" in npz:
            ...
            img = fallback_colors(capture, name, mask.shape)
    ...
    if os.path.exists(orig_ply_path):
        ...
        recon_v = world_points.reshape(-1, 3)[masks.reshape(-1)]
        assert orig_v.shape[0] == n_unfiltered, (
            f"point count mismatch vs scene.ply: {orig_v.shape[0]} vs {n_unfiltered}"
        )
        max_dev = float(np.abs(recon_v - orig_v).max())
        assert max_dev < 1e-3, f"coordinates deviate from scene.ply by {max_dev}"
        print(
            f"  verification vs scene.ply: {n_unfiltered} points match, "
    ...
    cols = np.clip(images.reshape(-1, 3)[flat_keep] * 255.0, 0, 255).astype(np.uint8)
    ...
    scene_glb = predictions_to_glb(
        {"world_points": world_points, "images": images, "final_masks": final_masks},
        as_mesh=False,
    )
    if show_cameras:
