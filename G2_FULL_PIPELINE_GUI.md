...
- Pipeline `unittest discover`：`106` 项通过，`2` 项仅因无显示跳过；
- Avoid 相关 pytest：`32` 项通过；
...
本次没有重新跑完整 CUDA MapAnything 推理，也没有连接真实 G2 相机；采集硬件链路需在已
source GDK 的机器人环境中执行。三份重建仍是粗粒度规划输入，不解除 Avoid 对未知实机夹爪、
